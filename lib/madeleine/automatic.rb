...
# Usage:
# class A
#   include MadeleineProxy
#   def initialize(param1, ...)
#   ...
#   def some_method(paramA, ...)
#   ...
# end
# mad = MadeleineAutomatic::System.start("storage_directory", A, param1, ...)
...
module MadeleineProxy
  class <<self
    def included(klass)
      class <<klass
        alias_method :_old_new, :new
        def new(*args, &block)
          MadeleineAutomatic::Prox.new(_old_new(*args, &block))
        end
      end
    end
  end
...
#
  class SysContainer
    ...
    def startMadeleine(where, what, *args)
      ...
      @madeleineSys = Madeleine::SnapshotMadeleine.new(where) { what.new(*args) }
      Thread.current[:in_command] = false
      @madeleineSys
    end
...
#
  class System
    ...
    def System.start(where, what, *args)
      System.instance.start(where, what, *args)
    end

    def start(where, what, *args)
      ...
      msys = scontainer.startMadeleine(where, what, *args)   # starts + gets the madeleine sys
