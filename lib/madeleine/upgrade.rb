...
#
    class Automatic_marshaller #:nodoc:
      ...
      def Automatic_marshaller.dump(obj, io = nil)
        Marshal.dump(obj)
        Thread.current[:system].marshaller.dump(Thread.current[:ao], io)
      end
    end
...
#
      def add(client_object)
        ...
        @root_proxy = @client_objects[0] if (@client_objects.size == 1)
    ...
    class Automatic_proxy #:nodoc:
      def initialize(client_object)
        @sysid = Thread.current[:system].sysid
        @myid = Thread.current[:ao].add(client_object)
      end
    end
#
...
#
      def take_snapshot
        begin
          ...
          Thread.current[:ao] = Automatic_objects.new
