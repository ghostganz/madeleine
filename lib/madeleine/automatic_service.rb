  ...
  class Automatic_service
    ...
    class << self
      ...
      def clean_storage(settings = nil)
        Madeleine_server.clean_storage(Automatic_service.settings(settings))
      end
      
      def instance(settings = nil)
        @@server = Madeleine_server.new(self, Automatic_service.settings(settings)) unless @@server
        @@server.system
      end
      
      def restart
        Automatic_service.clean_storage
